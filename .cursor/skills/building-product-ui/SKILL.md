...
| Command K grouping (server results via `GET /api/search`) | `src/client/app/command-palette.ts` |
...
**Inbox** — a two-pane chat, not a checklist ledger: `pageHeader` with the unread-thread count over a full-height `.inbox-shell` (one hairline frame, three independently scrolling columns). Left, `.inbox-list` thread rows (`data-thread-row`): initials avatar, client name, latest-message preview truncated at 96 chars with a `You:` prefix for CPA-latest, relative time, and a highlighter `.badge` unread count; the selected row washes `--surface-nav-active`; nothing is selected by default (module `inboxSelection`, so the 2s repaint keeps the selection). Center, the active conversation — or a quiet "Select a conversation" placeholder: a head (client name, `{filingType} · {taxYear}`, shared portal-link control), the scrolling `.inbox-messages` timeline (client bubbles left on `--surface-section`, CPA bubbles right on `--color-ink` with paper text, ~70% max width, 12px radius; quiet centered `.inbox-event` lines deep-linking `/documents/:id`), and the `.inbox-compose` form pinned at the bottom (`data-preserve-focus` so poll repaints never steal focus; drafts survive repaints via a module map keyed by engagementId). Right, `.inbox-files`: engagement summary, an Open portal link, and the thread's `documents` (served on the `/api/inbox` thread payload, newest first) as compact rows — filename link, `pipelineChip`, relative upload time. Send posts `POST /api/inbox/threads/:engagementId/messages`, clears the draft, and repaints from a fresh load; failures show the server's message verbatim in `[data-compose-error]`. Selecting an unread thread marks it read via `POST /api/inbox/threads/:engagementId/read`; unread = client messages + inbound events only — the CPA's own outbound never counts.
...
**Portal** — the chromeless client surface at `/portal/:token`. A letterhead header (32px ink firm monogram + firm name, 28px "Document request" title, one-line intro, and a "N of M received" progress line computed over non-waived checklist items — hidden when nothing is tracked) above three white `.portal-card` surfaces over the wash background (page max 1240px). Each card leads with a `.portal-panel-head` row (ink title + ash meta, hairline underneath) so the three column baselines align. The request checklist: required items first from the server; each item is one compact summary line — 20px circular status mark (success check / warning triangle / ash dash / dashed open), ink title, ink-outline `Required` micro chip, file count, and a chevron that rotates when the `<details>` disclosure opens — with description, nested files, and waive behind the disclosure whose open state survives poll repaints; one item collects n files of its type. The common `.dropzone` leads with a Feather `upload` mark on a wash disc, with the live upload list beneath (filename grows and truncates, chip right-aligned, hairline separators). The Messages panel renders light bubbles mirroring the inbox recipe from the client's side — firm left on `--surface-section`, "You" right on `--surface-wash`, hairline + input radius, micro-caption sender labels (no uppercase transform) — with the `data-preserve-focus` compose anchored behind a hairline rule posting `POST /api/portal/:token/messages` (module-level draft). Primary copy is `--color-ink` — ash on the bone background was a readability regression. Pipeline stages surface through the `.chip` recipe with client-facing labels (Done / Needs attention), never internal ones.
...
| Palette searches the whole database | Keystrokes debounce one `GET /api/search?q=` (clients, engagements, documents, document types; sequence-guarded against stale responses); Actions/Pages still filter locally — replaced the fetch-once shell index, which was incomplete and went stale | Search outgrows capped substring matching (needs ranking) |
