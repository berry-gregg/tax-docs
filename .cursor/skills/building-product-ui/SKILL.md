...
**Inbox** — request threads, not a flat ledger: `pageHeader` with the unread-thread count, then `.inbox-threads` disclosure sections — one per engagement's outbound request (client name, engagement label, "X of Y items received", portal-link control). Expanded body is one compact status line per request item (needs-attention → received → open → waived) deep-linking to `/documents/:id`; never a row per uploaded file. Expanding an unread thread marks it read via `POST /api/inbox/threads/:engagementId/read`.
