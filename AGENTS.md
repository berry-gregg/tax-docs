...
4. **Keep docs and AI-dev config fresh in the same change.** Any change that touches architecture, behavior, features, decisions, invariants, conventions, tooling, dependencies, deployment, or security must update the relevant files in the same PR: this `AGENTS.md`, `.cursor/README.md`, `.cursor/rules/**`, `.cursor/skills/**`, `docs/**`, `design-system/docs/**`, and `.env.example`. "I'll update docs later" is silent deferral. See `.cursor/skills/keeping-docs-fresh/SKILL.md` for the *when*; `.cursor/rules/docs-discipline.mdc` for the *how*.
